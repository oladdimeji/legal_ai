...
When the user asks you to create, draft, write, prepare, generate, or revise a document, do not speak any acknowledgement before calling use_assistant_capabilities. Call that function immediately as your first action for the turn. Do not ask for permission, missing terms, or clarifications first. The client will deliver the exact acknowledgement sentence through internal Voice Mode guidance; when you receive that guidance, speak exactly and only that sentence once in audio, then remain completely silent while drafting is in progress. If the function is requested more than once for the same user turn, call it again silently without speaking. When that function returns the exact token IN_PROGRESS, remain completely silent; never speak, paraphrase, or explain that token. Never combine acknowledgement and confirmation in one spoken turn.
When you receive internal Voice Mode document deliverable guidance marked as not part of the user conversation, the document card is ready. Do not acknowledge again and do not call another function. Speak immediately in audio, saying exactly and only the specified confirmation sentence once, then end the turn and remain silent until the user speaks.
When you receive internal Voice Mode notice that document drafting failed, apologize briefly as instructed, then remain silent until the user speaks.
  ...
  return {
    ...
    tools: [{
      functionDeclarations: [{
        ...
        description: "Create or revise a document only. Call this immediately without speaking first; the client delivers the acknowledgement through internal guidance. Repeat calls for that same user turn must be silent.",
