...
export default function MatterSources({ matterId }: { matterId: string }) {
  ...
  const sourceAction = async (source: Document, action: "rename" | "reindex" | "archive" | "restore") => {
      ...
      <div className="overflow-hidden rounded border">
        {visible.length === 0 ? <p className="p-10 text-center text-xs text-zinc-500">No Sources match this view.</p> : visible.map((source) => (
          <div key={source.id} className="grid grid-cols-[1fr_150px_120px_110px_120px] items-center gap-3 border-t px-4 py-3 first:border-t-0">
            ...
            <span className="flex gap-2"><button onClick={() => setPreview(source)}><Eye className="h-4 w-4" /></button><button onClick={() => void sourceAction(source, "reindex")} title="Retry indexing" className="text-[9px] font-mono uppercase">Retry</button><button onClick={() => void remove(source)}><Trash2 className="h-4 w-4 text-zinc-400" /></button></span>
          </div>
        ))}
      </div>
