...
interface Props { matters: Case[]; onRefresh: () => Promise<void> | void; onOpenMatter: (id: string) => void; resourceLifecycleEnabled?: boolean; }
type Sort = "activity" | "created" | "name";

export default function MattersView({ matters, onRefresh, onOpenMatter, resourceLifecycleEnabled = false }: Props) {
  ...
  useEffect(() => {
    if (!resourceLifecycleEnabled || !showArchived) return setLifecycleMatters(matters);
    void fetch("/api/cases?includeArchived=true").then(async (response) => {
      if (response.ok) setLifecycleMatters(await response.json());
    });
  }, [matters, resourceLifecycleEnabled, showArchived]);
  ...
  return <div className="flex-1 h-full overflow-y-auto bg-white p-8"><div className="mx-auto max-w-6xl space-y-6">
    <header className="flex items-center justify-between"><div><div className="flex items-center gap-2"><Briefcase className="h-5 w-5" /><h2 className="text-lg font-bold uppercase">Matters</h2></div><p className="mt-1 text-[11px] font-mono uppercase text-zinc-400">Matter workspaces and assignments</p></div><div className="flex items-center gap-3">{resourceLifecycleEnabled && <label className="text-[9px] font-mono uppercase"><input type="checkbox" checked={showArchived} onChange={(event) => setShowArchived(event.target.checked)} className="mr-1" />Show archived</label>}<button onClick={() => void openCreate()} className="flex items-center gap-2 rounded bg-zinc-950 px-4 py-2 text-[10px] font-mono font-bold uppercase text-white hover:bg-zinc-800 cursor-pointer"><Plus className="h-4 w-4" />Create Matter</button></div></header>
