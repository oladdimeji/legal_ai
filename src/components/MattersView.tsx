...
export default function MattersView({ matters, onRefresh, onOpenMatter }: Props) {
  ...
  </div>{showCreate && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-5"><form onSubmit={create} className="max-h-[92vh] w-full max-w-2xl space-y-4 overflow-y-auto rounded border border-zinc-300 bg-white p-6 shadow-xl"><div className="flex items-start justify-between"><div><h3 className="text-sm font-semibold uppercase">Create Matter</h3><p className="mt-1 text-xs text-zinc-500">Matter name and assignment description are required.</p></div><button type="button" disabled={saving} onClick={() => setShowCreate(false)} className="rounded p-1 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-900 disabled:cursor-not-allowed disabled:opacity-40"><X className="h-4 w-4" /></button></div><Input label="Matter name *" value={name} onChange={setName} /><label className="block text-[10px] font-mono font-bold uppercase text-zinc-500">Assignment description *<textarea value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1 h-28 w-full rounded border px-3 py-2 text-xs font-sans font-normal normal-case focus:outline-none focus:ring-1 focus:ring-zinc-400" /></label><div className="grid gap-3 md:grid-cols-2"><label className="flex cursor-pointer items-center justify-between rounded border border-zinc-200 px-3 py-2 text-xs hover:bg-zinc-50"><span className="flex items-center gap-2"><Upload className="h-4 w-4" />Optional files</span><span className="text-[10px] text-zinc-500">{fileSelection.files.length ? fileSelection.selectedLabel : ""}</span><input type="file" disabled={saving} className="sr-only" multiple accept=".pdf,.docx,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" onChange={(event) => { fileSelection.addFiles(event.target.files); event.currentTarget.value = ""; }} /></label><div className="relative"><button type="button" disabled={saving} onClick={() => setLibraryOpen(!libraryOpen)} className="flex w-full items-center justify-between rounded border border-zinc-200 px-3 py-2 text-xs hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-40"><span>{selectedLibrary.length ? `${selectedLibrary.length} Firm Library selected` : "Optional Firm Library"}</span><ChevronDown className="h-4 w-4" /></button>{libraryOpen && <div className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded border bg-white p-2 shadow-lg">{library.length === 0 ? <p className="p-3 text-xs text-zinc-400">No Firm Library documents.</p> : library.map((document) => <label key={document.id} className="flex items-center gap-2 rounded px-2 py-2 text-xs hover:bg-zinc-50"><input type="checkbox" disabled={saving} checked={selectedLibrary.includes(document.id)} onChange={(e) => setSelectedLibrary((current) => e.target.checked ? Array.from(new Set([...current, document.id])) : current.filter((id) => id !== document.id))} />{document.title}</label>)}</div>}</div></div>{fileSelection.fileError && <p className="text-xs text-red-700">{fileSelection.fileError}</p>}<SelectedFileList files={fileSelection.files} onRemove={fileSelection.removeFile} disabled={saving} />{sourceFailures.map((failure) => <p key={failure.identity} className="text-xs text-red-700">{failure.file.name}: {failure.error}</p>)}<div className="flex items-center justify-between gap-2"><span className="text-xs text-zinc-500">{status}</span><div className="flex gap-2"><button type="button" disabled={saving} onClick={() => { reset(); setShowCreate(false); }} className="rounded border px-4 py-2 text-[10px] font-mono uppercase hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-40">Cancel</button><button disabled={saving || !name.trim() || !description.trim()} className="inline-flex items-center gap-2 rounded bg-zinc-950 px-4 py-2 text-[10px] font-mono font-bold uppercase text-white disabled:cursor-not-allowed disabled:opacity-40">{saving && <Check className="h-3.5 w-3.5 animate-pulse" />}{saving ? status || "Creating..." : "Create Matter"}</button></div></div></form></div>}</div>;
}
