...
export default function ClientPortalView({ token }: { token: string }) {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-6"><div className="flex max-h-[85vh] w-full max-w-3xl flex-col rounded border bg-white"><div className="shrink-0 bg-white p-6 pb-3"><button onClick={() => setOpen(null)} className="float-right rounded p-1 hover:bg-zinc-100"><X className="h-4 w-4" /></button><h3 className="font-semibold">{open.title}</h3></div><div className="min-h-0 flex-1 overflow-y-auto bg-white px-6 py-3"><WorkProductDocument content={open.content} /></div><div className="shrink-0 bg-white p-6 pt-3"><div className="flex gap-2"><input value={comment} onChange={(event) => setComment(event.target.value)} className="flex-1 rounded border px-3 py-2 text-xs" placeholder="Add a comment" /><button onClick={() => void addComment()} className="flex items-center gap-1 rounded border px-4 py-2 text-[9px] font-mono font-bold uppercase hover:bg-zinc-50"><MessageSquare className="h-3.5 w-3.5" />Comment</button></div></div></div></div>}
