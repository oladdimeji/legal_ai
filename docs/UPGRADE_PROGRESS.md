...
- The timer is scheduled when confirmation is dispatched, cancelled on successful confirmation completion, contentless `turnComplete`, interruption, and session teardown.
- Changed files: `src/lib/voiceDocumentConfirmation.ts`, `src/hooks/useVoiceMode.ts`, `tests/assistant-voice-mode.test.ts`.
...
### Voice document-turn completion and post-draft conversation isolation

- Fixed the post-document turn boundary that could consume the next ordinary Voice response. Document finalization previously set `voiceDocumentJustFinalizedRef` from an asynchronous playback-drain callback; because no Live message handler remained to clear it, the next real `turnComplete` returned early without finalizing that request and response. The cross-event flag is removed. Finalization is now a local result of the exact Live event being handled, so a completed document can never suppress the following conversation turn.
- Confirmation completion now requires confirmation audio to have actually started, in addition to dispatch, Live turn completion, and idle playback. A late `turnComplete` from the request or acknowledgement can no longer masquerade as completion of confirmation audio that has not arrived. The existing 4.5-second failsafe remains the recovery path when Live produces no confirmation audio.
- Restored an independent completion path for proactive document drafting. Draft generation still starts at the same early transcript point and therefore retains its existing speed, but its result is now recorded by turn. If Live omits or delays `use_assistant_capabilities`, the completed input turn delivers the ready document and confirmation instead of leaving document capture and transcript suppression active forever.
- Tool-call delivery and completed-input delivery share one idempotent function and the existing delivered-turn set. A late or duplicate tool call receives the immediate in-progress response without replaying acknowledgement, confirmation, persistence, or the document card.
- Proactive completion is held until either the matching tool call or the user's Live input turn completes, so a fast draft cannot interrupt a still-open spoken request. Ordinary Voice conversations do not enter this document-only state and their capture, transcription, persistence, playback, and rendering paths are unchanged.
- Failed document turns now clear the locked document transcript and expected spoken strings before returning to ordinary conversation, preventing stale document state from contaminating a retry or follow-up.
- Changed files: `src/hooks/useVoiceMode.ts`, `tests/assistant-voice-mode.test.ts`, `docs/UPGRADE_PROGRESS.md`.
- Schema changes: none. No migration, dependency, server route, drafting prompt, drafting model, database query, authentication, or workspace-scoping change was made.
- Assumption: Gemini Live may omit, duplicate, or reorder tool and completion events, so browser document delivery must be idempotent and must not depend on one particular event ordering.
- Remaining issue: automated tests deterministically cover the lifecycle guards and integration wiring, but final microphone/speaker acceptance still requires a live browser session because Gemini Live event timing and device audio cannot be reproduced by the Node test runner.
- Verification: `npm run lint` passed, the focused Voice/document suite passed 51/51, all 504 repository tests passed, and `npm run build` passed with the existing non-fatal large-chunk warning. The first sandboxed build attempt was blocked from reading Vite's config by the managed filesystem boundary; the required build passed unchanged outside that restriction.

### Voice document acknowledgement leak and silent-handoff recovery

- Confirmed the leaked `now.` bubble as a late suffix-only output-transcription event from the exact scripted document acknowledgement. The previous guard recognised the full sentence and its opening prefix, but document finalization cleared the expected sentence before Gemini Live delivered this trailing fragment, allowing it to become an ordinary assistant response.
- Document acknowledgement and confirmation strings now remain in a five-second, document-only speech guard after finalization. Matching is punctuation-insensitive and recognises bounded contiguous fragments of those exact strings, so late tails such as `now.`, `for you.`, or `the NDA.` are discarded. The guard is cleared as soon as the next genuine user transcription begins, preserving every normal follow-up response.
- Confirmed the silent freeze as an unbounded state between a ready document and confirmation dispatch. Confirmation intentionally waits for acknowledgement playback to drain, but the AudioWorklet drain signal can be delayed or missed; the existing 4.5-second failsafe began only after confirmation dispatch, so it never covered this queued handoff.
- Added a 6.5-second document-handoff watchdog as soon as a completed draft queues its confirmation. If playback never becomes idle, recovery cancels only the stuck internal audio, invalidates deferred acknowledgement/confirmation animation-frame callbacks, finalizes the exact pending document once, preserves request-then-response ordering, and reopens capture. The existing post-dispatch 4.5-second confirmation watchdog remains unchanged.
- Recovery is not extended by slow transcript storage: optimistic document messages are already ordered in chat, so only this exceptional watchdog path may resume capture while their idempotent persistence queue finishes in the background. Persistence requests have a ten-second abort boundary and one retry using the same deterministic event ID. Ordinary conversation and successful document turns retain their existing capture and persistence timing.
- Voice shutdown, restart, failure, and component cleanup now synchronously flush a ready generated document into the optimistic/persistence path before clearing session and live-card state. Late draft promises and deferred speech callbacks revalidate their turn, session, and dispatch sequence, so an old session cannot delete, restore, speak, or attach a document in a newer session.
- Changed files: `src/hooks/useVoiceMode.ts`, `src/lib/voiceAcknowledgement.ts`, `src/lib/voiceDocumentConfirmation.ts`, `tests/assistant-voice-mode.test.ts`, `tests/voice-acknowledgement.test.ts`, `docs/UPGRADE_PROGRESS.md`.
- Schema changes: none. No migration, dependency, server route, drafting pipeline, drafting prompt, model, database query, authentication, workspace scope, regular conversation rendering, or successful document-generation speed changed.
- Assumptions: Gemini Live output transcription and completion events may arrive after audio drain or finalization, and a browser AudioWorklet drain notification may be delayed or absent. The client therefore treats those callbacks as reorderable and bounds every ready-document handoff.
- Remaining issue: deterministic tests cover the late-fragment, stale-callback, shutdown flush, persistence timeout, and both confirmation watchdog paths. Final microphone/speaker acceptance still requires a live browser session because device audio and Gemini Live timing cannot be reproduced in the Node runner.
- Verification: `npm run lint` passed, the focused Voice/document suite passed 51/51, all 504 repository tests passed, and `npm run build` passed with the existing non-fatal large-chunk warning. The first sandboxed build attempt was blocked from reading Vite's config by the managed filesystem boundary; the required build passed unchanged outside that restriction.

### Voice document single-shot confirmation and terminal audio drain

- Confirmed the duplicated confirmation was not a duplicate client dispatch. The client invalidated the pending confirmation before dispatch and marked its turn delivered, but it asked Gemini Live through a natural-language prompt to speak the confirmation once. A generative Live turn has no deterministic one-utterance boundary: it can repeat the sentence or return additional audio packets even though the browser issued only one request.
- Document confirmation is now a deterministic, prepared Kore speech clip from the existing authenticated `/voice/speak` route. It is prefetched in parallel with the unchanged document request, retained as one promise per document turn, and scheduled locally exactly once after the card is ready and acknowledgement playback is idle. Gemini Live remains silent for this internal confirmation turn, and any late Live document audio is discarded while the prepared clip owns playback.
- Confirmed the silent freeze in the playback worklet. After the normal 300 ms prebuffer had drained, `playing` became false. A late terminal packet shorter than the prebuffer could then enter the queue, set the hook's `playbackActive` state, and never play or emit `drained`: the non-playing branch returned before the drain condition and waited forever for a threshold that no more packets would satisfy. This permanently held document confirmation, microphone capture, and the UI without producing a JavaScript error.
- The playback protocol now has explicit `begin-stream` and `finish-stream` boundaries. Finishing forces a valid short terminal clip to play below the normal prebuffer threshold, and non-stream late packets receive a bounded priming window before playback starts. A closed, empty stream always emits `drained`, so `playbackActive` cannot remain latched by a stranded tail. Interruption and shutdown still clear the queue immediately.
- Added an executable AudioWorklet regression simulation. It proves a prepared short confirmation drains once and that a late packet below the prebuffer threshold, which remained stuck under the previous implementation, now plays and emits its own drain event. Production-source assertions also prove confirmation uses one prepared schedule and no Live confirmation dispatch.
- The existing 6.5-second ready-document handoff watchdog and 4.5-second post-dispatch playback watchdog remain as independent recovery boundaries. Missing or failed prepared audio abandons only confirmation playback and finalizes the ready document without freezing capture.
- Changed files: `server/voiceMode.ts`, `src/hooks/useVoiceMode.ts`, `src/lib/voicePlaybackWorklet.ts`, `tests/assistant-voice-mode.test.ts`, `docs/UPGRADE_PROGRESS.md`.
- Schema changes: none. No migration, dependency, database query, authentication, workspace scope, drafting route, drafting planner, drafting prompt, drafting model, regular Voice conversation path, or document-generation path changed. Confirmation audio is prefetched beside drafting and does not block or slow document creation.
- Assumption: browser `MessagePort` messages sent to one AudioWorklet arrive in order, as required by the Web platform contract. Final microphone/speaker acceptance still requires a live browser because physical device audio and Gemini Live network timing cannot be reproduced in the Node runner.
- Verification: `npm run lint` passed, the focused Voice/document suite passed 52/52, all 505 repository tests passed, and `npm run build` passed with the existing non-fatal large-chunk warning. The first sandboxed build attempt was blocked from reading Vite's config by the managed filesystem boundary; the identical build passed unchanged outside that restriction.

### Voice document confirmation ownership and durable-card boundary

- Confirmed the remaining mid-confirmation cutoff was a different lifecycle path from the repaired worklet tail. The confirmation is client-owned audio, but a delayed Gemini Live `interrupted` event from the earlier acknowledgement/tool turn still entered the generic interruption handler. That handler called `stopPlayback()` before checking confirmation ownership, so the stale event could cut the local confirmation mid-word.
- The same interruption path finalized the document without enabling the document recovery persistence bypass. Voice capture therefore remained held behind the ordered persistence queue; with the existing ten-second request boundary and one retry per queued message, the UI could remain silent for a prolonged period with no JavaScript error. This is why stopping and restarting Voice appeared to be the only recovery.
- Prepared confirmation now has exclusive playback ownership. Microphone capture is already held during that clip, so a Live `interrupted` event received in that window cannot be a legitimate barge-in and is ignored. Ordinary Live responses retain their existing interruption behavior and still stop immediately on a genuine Live interruption.
- Prepared confirmation no longer uses the persistent packet-stream worklet. The complete PCM response is decoded into one finite `AudioBufferSourceNode`, whose `ended` event is its exact completion boundary. Packetized ordinary Live audio continues to use the unchanged jitter-buffer worklet.
- Removed the fixed-duration truncation hazard. The watchdog now uses the decoded clip duration plus a two-second recovery margin, with 4.5 seconds retained only as its minimum. Its callback validates the immutable dispatch sequence and does not recheck `Date.now()`, so a wall-clock adjustment cannot consume the only recovery callback without resolving the turn.
- The speech endpoint previously selected only the first audio part returned by Gemini. Compatible multipart PCM is now validated and concatenated in order, preventing a multipart response from becoming a partial spoken sentence; incompatible part formats fail safely instead of producing corrupt playback.
- The ready document is now finalized into the optimistic and idempotent persistence path before confirmation playback starts. Audio completion is no longer allowed to own document durability, so stopping Voice during a browser audio fault cannot remove the generated card. Persistence continues in its existing ordered background queue.
- Changed files: `server/voiceMode.ts`, `src/hooks/useVoiceMode.ts`, `src/lib/voiceDocumentConfirmation.ts`, `tests/assistant-voice-mode.test.ts`, `docs/UPGRADE_PROGRESS.md`.
- Schema changes: none. No migration, dependency, database query, authentication, workspace scope, drafting route, drafting planner, drafting prompt, drafting model, regular Voice generation, or document-generation speed changed.
- Assumption: Gemini Live control events may arrive after the acknowledgement/tool turn they describe. This is already an established property of the event boundary, so client-owned confirmation audio is isolated by ownership rather than event arrival order.
- Remaining issue: deterministic tests cover interruption ownership, duration-aware dispatch recovery, multipart audio assembly, durable-card-before-audio ordering, finite-source completion, and the existing worklet tail. Final physical microphone/speaker acceptance still requires a live browser session.
- Verification: `npm run lint` passed, the focused Voice/document suite passed 53/53, all 506 repository tests passed, and `npm run build` passed with the existing non-fatal large-chunk warning. The first sandboxed build attempt was blocked from reading Vite's config by the managed filesystem boundary; the identical build passed unchanged outside that restriction.
